...
BET_CFG = {"over_under_lines": [0.5, 1.5, 2.5], "handicap_lines": [-0.5, 0.5], "top_scores": 5,
...
def _base():
    return Predictor.from_matches(
        _base_matches(), model_config=MODEL_CFG, mc_config=MC_CFG,
        canonical=lambda x: x, as_of=pd.Timestamp("2020-01-01"),
        betting_config=BET_CFG, betting_config_version="testfp")
...
def test_live_predictor_preserves_betting_config_so_markets_work():
    base = _base()
    played = [{"team_a": "Brazil", "team_b": "Haiti", "goals_a": 3, "goals_b": 0}] * 4
    live = build_live_predictor(base, played, tournament_date="2026-06-15",
                                model_config=MODEL_CFG, mc_config=MC_CFG)
    assert live.betting_config == BET_CFG
    # Markets must work on the LIVE model (regression: betting_config was missing).
    out = live.predict("Brazil", "Haiti", neutral=True, include_markets=True)
